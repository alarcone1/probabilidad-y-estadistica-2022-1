...
- Si $X_1, X_2,...,X_m$ son **variables aleatorias independientes**, $\textsf V[\sum_{i=1}^{m}X_j]=\sum_{i=1}^{m}\textsf V[X_j]$.
...
$$
f(x) =\frac{\theta\,\eta^\theta}{x^{\theta+1}}\,,\quad\text{para $x>\eta$,}
$$
...
b. Demostrar que $f_X(x)$ es una función de densidad autentica, es decir, demostrar que: 
	$$
	\text{$f(x)\geq 0$ para todo $x>1$}
	\qquad\text{y}
	\qquad\int_{-\infty}^{\infty}f(x)\,\text{d}x = 1.
	$$
